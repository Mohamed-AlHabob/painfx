...
export default function ProflilePage({ userId }: DoctorInfoPageProps) {
  // Fetch doctor data
  const { data, isLoading,isFetching ,error} = useRetrieveUserQuery();
  const userData = (data as UserProfile) || {};
  const { onOpen } = useModal();

  // Handle loading and error states
  if (isLoading) {
    return <ProflilePage.Skeleton />;
  }
      ...
      <div className="relative overflow-hidden rounded-lg p-8">
        ...
        <div className="relative flex flex-col items-center sm:items-start sm:flex-row gap-6">
          <div className="relative">
            <div className="h-32 w-32 rounded-full p-1 bg-yellow-400">
              <Avatar>
                ...
                />
                {/* <AvatarFallback>{userData?.first_name?.charAt(0) || ""}</AvatarFallback> */}
              </Avatar>
            </div>
            <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 flex items-center gap-1 bg-yellow-400/90 px-3 py-0.5 rounded-full">
              ...
              <span className="text-xs font-medium">
                {/* {userData?.specialization?.name || "Specialization not provided"} */}
          ...
          <div className="flex-1 text-center sm:text-left">
            <div className="text-sm">{userData?.email || "Email not provided"}</div>
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-2">
            {/* <Button
              ...
              disabled={!userData?.reservation_open}
              className={`flex items-center ${
                !userData?.reservation_open ? "opacity-50 cursor-not-allowed" : ""
              }`}
            >
              <span
                className={`h-2 w-2 rounded-full ${
                  userData?.reservation_open ? "bg-green-400" : "bg-orange-400"
                }`}
              />
              <span>
                {userData?.reservation_open
                  ? "Reservation"
                  : "Reservation Closed"}
              </span>
            </Button> */}
...
ProflilePage.Skeleton = function ProflilePageSkeleton() {
  return (
    <div className="relative">
      {/* Header Skeleton */}
      <div className="relative overflow-hidden rounded-lg p-8">
